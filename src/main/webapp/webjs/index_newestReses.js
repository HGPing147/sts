$(function() {
	...
	var showNewestRes = function(accessToken) {
		$.ajax({
			url : 'api/v1/reses/newest',
			type : 'GET',
			async : false,
			data : {
			...
			success : function(data) {
				var newestReses = data.data;
				var resesArr = [];
				if (newestReses.length > 0) {
					$.each(newestReses, function(i, d) {
						var resId = d.id;
					...
					$('#next_c1').show();
					$('#prev_c1').show();
					$('.newest .list_product').html(resesArr.join(' '));
					showResDetail(accessToken);// 商品详情
					showUserContact(accessToken);// 用户联系方式
					buy(accessToken);// 购买
				} else {
					$('#next_c1').hide();
					$('#prev_c1').hide();
					$('.newest .list_product').html('');
				}
			},
			error : function() {
				$('#next_c1').hide();
